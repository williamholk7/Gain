...
void GainAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float gainValue = *apvts.getRawParameterValue(g_Gain);
...
juce::AudioProcessorValueTreeState::ParameterLayout GainAudioProcessor::createParams()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
    
    auto dBAttributes = juce::AudioParameterFloatAttributes().withLabel(" dB");
    
    params.push_back(std::make_unique<juce::AudioParameterFloat>(juce::ParameterID{g_Gain, kParameterVersionHint}, "Gain", juce::NormalisableRange<float>(-12.f, 12.f, .1f), 0.f), dBAttributes);
}
